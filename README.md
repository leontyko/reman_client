...
Команды происходят по GET-запросу из браузера или другого приложения по API
...
3. Для упралением уровнем громкости используется запрос вида: http://<IP-адрес>:<Port>/volume?cmd=up&point=3
В параметр 'cmd' принимаются следующие значения:
'up' для увеличения
'down' для уменьшения
...
6. Для проверки доступности нужен запрос: http://<IP-адрес>:<Port>
